...
public:
    ...
    {
        // maxval : 如果number进来，新窗口的最大值
        int maxval = maxh < maxt ? max(arr[maxDeque[maxh]], number) : number;
        // minval : 如果number进来，新窗口的最小值
        int minval = minh < mint ? min(arr[minDeque[minh]], number) : number;
