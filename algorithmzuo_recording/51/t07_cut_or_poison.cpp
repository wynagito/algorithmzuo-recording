...
// hp：怪兽血量
// limit：回合的限制
bool f(const vector<int> &cuts, const vector<int> &poisons, long hp, int limit)
{
    int n = min(static_cast<int>(cuts.size()), limit);
    for (int i = 0, j = 1; i < n; i++, j++)
    {
        hp -= max(static_cast<long>(cuts[i]), static_cast<long>(limit - j) * static_cast<long>(poisons[i]));
        if (hp <= 0)
        {
            return true;
        }
