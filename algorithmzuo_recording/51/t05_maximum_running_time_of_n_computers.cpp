...
public:
    ...
    long long maxRunTime1(int num, vector<int> &arr)
    {
        long long sum = 0;
        for (int x : arr)
        {
    ...
    bool f1(vector<int> &arr, int num, long long time)
    {
        // 碎片电量总和
        long long sum = 0;
        for (int x : arr)
    ...
    // 二分答案法 + 增加分析(贪心)
    // 时间复杂度O(n * log(maxval))，额外空间复杂度O(1)
    long long maxRunTime2(int num, vector<int> &arr)
    {
        int maxval = 0;
        long long sum = 0;
        for (int x : arr)
        ...
        {
            // 所有电池的最大电量是maxval
            // 如果此时sum > (long) maxval * num，
            // 说明 : 最终的供电时间一定在 >= maxval，而如果最终的供电时间 >= maxval
            // 说明 : 对于最终的答案X来说，所有电池都是课上讲的"碎片拼接"的概念
            // 那么寻找 ? * num <= sum 的情况中，尽量大的 ? 即可
            // 即sum / num
            return sum / num;
        }
        // 最终的供电时间一定在 < maxval范围上
    ...
    bool f2(vector<int> &arr, int num, long long time)
    {
        // 碎片电量总和
        long long sum = 0;
        for (int x : arr)
